...
export const useGameLogic = ({ 
  ...
  const handleCellClick = useCallback((row, col) => {
    ...
    console.log(`[落子] 步骤${moveHistory.length + 1}: ${currentPlayer} 落子 (${row}, ${col})`);
    console.log('[落子] 保存的board:', JSON.stringify(moveRecord.board.map(r => r.map(c => c || 0))));
  ...
  useEffect(() => {
    ...
    if (bestMove) {
      ...
      console.log('[AI落子] newBoard 落子后的棋盘:', JSON.stringify(newBoard.map(r => r.map(c => c || 0))));
      setBoard(newBoard)
      ...
      console.log(`[AI落子] 步骤${moveHistory.length + 1}: ${AI_PLAYER} 落子 (${bestMove.row}, ${bestMove.col})`);
      console.log('[AI落子] 保存的board:', JSON.stringify(aiMoveRecord.board.map(r => r.map(c => c || 0))));
