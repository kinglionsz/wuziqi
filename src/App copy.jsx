...
function App() {
  ...
  const replayStep = useCallback((step) => {
    ...
    if (step < 0) {
      ...
      // 【调试日志】更详细的回放日志
      console.log(`========== [回放] 步骤 ${step + 1} ==========`);
      console.log('[回放] moveHistory 完整记录:');
      moveHistory.forEach((m, i) => {
        console.log(`  步骤${i+1}: player=${m.player}, position=(${m.position.row}, ${m.position.col})`);
      });
      console.log('[回放] 重建棋盘后的棋子位置:');
      newBoard.forEach((row, r) => {
        row.forEach((cell, c) => {
          if (cell) console.log(`  (${r},${c}) = ${cell}`);
        });
      });
      console.log('[回放] targetMove:', targetMove);
      setBoard(newBoard)
      // 恢复该步的玩家（下一步应该是对方）
