...
| 参数 | 说明 | 示例 |
|------|------|------|
| action | 操作类型 | `deploy` / `query` |
| serverName | 云托管服务名称 | `wuziqi-server` |
| serverType | 服务类型 | `container` (容器型) |
| targetPath | 本地代码绝对路径 | `D:\...\cloudbase\server` |
| Cpu | CPU 核数 | `0.5` / `1` / `2` |
| Mem | 内存 GB (需是 CPU 的 2 倍) | `1` / `2` / `4` |
| MinNum | 最小实例数 | `0` / `1` |
| MaxNum | 最大实例数 | `3` / `5` |
| Port | 容器监听端口 | `3000` |
| OpenAccessTypes | 访问类型 | `PUBLIC` / `OA` / `MINIAPP` |

**MCP 工具列表**:

| 工具名 | 功能 |
|--------|------|
| `mcp__cloudbase__envQuery` | 查询环境信息 |
| `mcp__cloudbase__manageCloudRun` | 云托管服务管理 (部署/更新/删除) |
| `mcp__cloudbase__queryCloudRun` | 查询云托管服务详情 |
| `mcp__cloudbase__uploadFiles` | 上传文件到静态托管 |
| `mcp__cloudbase__deleteFiles` | 删除静态托管文件 |
